...
GENUS_DESCS = {
    "Chitrobacter":  "Gram-negative, facultatively anaerobic rods. Found in soil, water, and intestinal tract. Associated with opportunistic infections in immunocompromised patients.",
    "Enterobacter":  "Gram-negative, motile rods. Common environmental and gut bacterium. Important nosocomial pathogen with emerging antibiotic resistance.",
    "Erwinia":       "Gram-negative rods primarily causing plant diseases including fire blight, soft rot, and wilts in many economically important crops.",
    "Escherichia":   "Gram-negative, facultatively anaerobic rods. Most strains are harmless gut commensals; pathogenic strains cause UTIs, diarrhea, and septicemia.",
    "Klebsiella":    "Gram-negative, non-motile, encapsulated rods. Major cause of hospital-acquired pneumonia, UTIs, and bloodstream infections.",
    "Morganella":    "Gram-negative rods found in intestinal tracts. Associated with UTIs, wound infections, and nosocomial bacteremia.",
    "proteus":       "Gram-negative, highly motile rods showing swarming motility. Common cause of catheter-associated UTIs and wound infections.",
    "Providencia":   "Gram-negative rods related to Proteus. Associated with UTIs and nosocomial infections in burned and catheterized patients.",
    "Salmonella":    "Gram-negative rods, major foodborne pathogens. Cause salmonellosis, typhoid fever, and paratyphoid fever worldwide.",
    "Serratia":      "Gram-negative rods; some strains produce red pigment prodigiosin. Important ICU pathogen with high antibiotic resistance rates.",
    "shigella":      "Gram-negative, non-motile rods. Cause shigellosis (bacillary dysentery), a leading cause of diarrheal mortality globally.",
...
.hdr p{font-size:11px;color:#90caf9;margin-top:2px}
...
.srch input:focus{outline:none;border-color:#1565c0}
...
<div class="stats">
  ...
  <div>LOO-CV: <b>{{summary.loo_accuracy}}%</b></div>
</div>
<div class="main">
<div>
  <div class="card">
    ...
    <div class="quick">
      {% for t in key_tests %}
      <div class="qtag" onclick="quickAdd('{{t}}')">{{t}}</div>
      {% endfor %}
    </div>
  </div>
  <div class="card">
      ...
      <div class="tgrid" id="tgrid">
        ...
        <div class="trow active" id="row-{{loop.index}}">
          ...
          <select name="test_{{t}}" class="tsel" onchange="markA(this,'row-{{loop.index}}')">
            {% for sym in ['+','-','(+)','(-)','d','ND'] %}
            <option value="{{sym}}" {{'selected' if v==sym else ''}}>{{sym}}</option>
            {% endfor %}
          </select>
    ...
    <button type="button" id="runbtn" class="runbtn" onclick="document.getElementById('frm').submit()" disabled>
      &#128300; Identify Organism
    </button>
    <button type="button" class="clrbtn" onclick="clearAll()">Clear all</button>
    <div class="hint">&#11088; = key diagnostic tests &bull; Enter at least 3 tests</div>
  </div>
</div>
<div>
{% if result %}
...
<div class="card">
  ...
  <div id="t1" class="tp on">
    <div class="cands">
      ...
      <div class="cand {{'top' if loop.first else ''}}">
        ...
        <div style="flex:1">
          <div class="cname">{{c.organism}}</div>
          <div class="csub">{{c.genus}}</div>
        </div>
  ...
  <div id="t3" class="tp">
    ...
    <div class="org-chips">
      {% for o in result.genus_orgs %}
      <span class="ochip {{'me' if o==result.organism else ''}}">{{o}}</span>
      {% endfor %}
    </div>
  </div>
  <div id="t4" class="tp">
...
<div class="card">
  <div class="empty">
    ...
    <div style="font-size:13px;color:#b0bec5;max-width:340px;margin:0 auto;line-height:1.8">
      Enter biochemical test results on the left and click Identify Organism.<br><br>
      Use the quick-add buttons or type any test name. Enter at least 3 tests for best accuracy.
    </div>
  </div>
</div>
{% endif %}
...
function updateCount(){
  var c=document.querySelectorAll('.trow').length;
  document.getElementById('cnt').textContent=c;
  document.getElementById('runbtn').disabled=c<3;
}
function quickAdd(name){
  var ex=document.querySelectorAll('.tname');
  for(var i=0;i<ex.length;i++) if(ex[i].textContent===name) return;
  addRowHTML(name,'d'); updateCount();
}
function addTest(){
  var name=document.getElementById('newtname').value.trim();
  var sym=document.getElementById('newtsym').value;
  if(!name) return;
  var ex=document.querySelectorAll('.tname');
  for(var i=0;i<ex.length;i++) if(ex[i].textContent===name) return;
  addRowHTML(name,sym);
...
function addRowHTML(name,sym){
  rowCount++;
  var id='row-'+rowCount;
  var opts=['+','-','(+)','(-)','d','ND'].map(function(v){
    return '<option value="'+v+'"'+(v===sym?' selected':'')+'>'+v+'</option>';
  }).join('');
  var div=document.createElement('div');
  div.className='trow'+(sym!=='d'&&sym!=='ND'?' active':'');
  div.id=id;
  div.innerHTML='<span class="tname">'+name+'</span>'
    +'<select name="test_'+name+'" class="tsel" onchange="markA(this,\''+id+'\')">'+opts+'</select>'
    +'<button type="button" class="delbtn" onclick="delRow(this)">&#10005;</button>'
    +'<input type="hidden" name="testname_'+rowCount+'" value="'+name+'">';
...
function sw(btn,tid){
  ...
  card.querySelectorAll('.tab').forEach(function(b){b.classList.remove('on')});
  card.querySelectorAll('.tp').forEach(function(p){p.classList.remove('on')});
  btn.classList.add('on');
  var el=document.getElementById(tid);if(el) el.classList.add('on');
}
function filterTbl(){
  var q=document.getElementById('psrch').value.toLowerCase();
  document.querySelectorAll('#ptbl tbody tr').forEach(function(r){
...
def identify():
    ...
    i = 1
    while True:
        tname = request.form.get(f"testname_{i}")
        if not tname:
            break
        tval = request.form.get(f"test_{tname}", "d")
    ...
    # Organism attribute counts
    profile_data = PROFILES.get(result["organism"], {})
    pos_tests = profile_data.get("pos_tests", [])
    neg_tests = profile_data.get("neg_tests", [])
    var_tests = profile_data.get("var_tests", [])
    result["pos_tests"]  = pos_tests
    result["neg_tests"]  = neg_tests
    result["var_tests"]  = var_tests
    result["pos_count"]  = len(pos_tests)
    result["neg_count"]  = len(neg_tests)
    result["var_count"]  = len(var_tests)
